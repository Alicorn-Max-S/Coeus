#!/usr/bin/env python3
"""Canvas LMS API CLI for nanobot.
...
def _load_env_value(key: str) -> str:
    """Load a value from ~/.nanobot/.env if not already in os.environ."""
    val = os.environ.get(key, "")
    if val:
        return val
    env_path = os.path.join(
        os.environ.get("NANOBOT_HOME", os.path.join(os.path.expanduser("~"), ".nanobot")),
        ".env",
    )
    try:
...
CANVAS_API_TOKEN = _load_env_value("CANVAS_API_TOKEN")
CANVAS_BASE_URL = _load_env_value("CANVAS_BASE_URL").rstrip("/")
...
def _check_config():
    ...
    if missing:
        print(
            ...
            "Set them in ~/.nanobot/.env or export them in your shell.\n"
...
def _db_path():
    nanobot_home = os.environ.get(
        "NANOBOT_HOME", os.path.join(os.path.expanduser("~"), ".nanobot")
    )
    return os.path.join(nanobot_home, "canvas_assignments.db")
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for nanobot"
    )
